    ...
    let data = allUsers
      ...
      .map((otheruser) => (
        <ProfileComponent
          ...
          user={user}
          router={router}
          maindiv=" w-[150px] shadow-md shadow-yellow-500 p-4 rounded-md my-2 mx-6 h-[135px] hover:scale-110 transition-transform duration-75 border border-t-neutral-300"
    ...
    let data = allGigs?.gigs
      ...
      .map((gig) => (
        <GigDisplay
          ...
          thirdDiv="xl:w-[300px] flex items-center justify-between bg-gray-300  shadow-md shadow-yellow-500 rounded-md h-[100px] whitespace-nowrap  p-1 min-w-[190px]  hover:bg-gray-400 transition-all tansition-transform hover:scale-110 ease-in-out duration-75"
          title="text-[13px] font-bold text-neutral-700"
          imageno={27}
        />
      ));
